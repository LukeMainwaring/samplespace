...
- **Pair evaluation upgrades** (complete): Two distinct workflows — ad-hoc pair evaluation (user picks a specific sample, `is_loop` filtering from user language) and pairing sessions (rapid-fire with random anchors, agent auto-presents next pair after each verdict). CLAP-based context-aware retrieval (shared with kit builder), "Play Together" mixed audio preview, scoring summary display, recently evaluated samples excluded from both anchors and candidates. Verdict messages render as compact pills in the chat UI.
